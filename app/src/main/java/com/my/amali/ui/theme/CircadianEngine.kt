...
data class LightProfile(
    ...
    /**
     * Словесная ступень освещения — показывается пользователю, чтобы
     * адаптация была объяснимой, а не «магической».
     *
     * Возвращается **не строка**, а перечисление: слой логики не имеет права
     * знать про язык интерфейса. Раньше здесь стоял русский текст, и при
     * любой из восьми не-русских локалей приложения пользователь видел
     * «Закат» посреди немецкого интерфейса. Текст — дело ресурсов
     * ([com.my.amali.R.string.light_candle] и соседей), а здесь остаётся
     * только решение «какая сейчас ступень».
     */
    val lightLabel: LightingLabel
        get() = when {
            cct <= 2100 -> LightingLabel.CANDLE
            cct <= 2500 -> LightingLabel.WARM_LAMP
            cct <= 3100 -> LightingLabel.SUNDOWN
            cct <= 3900 -> LightingLabel.EVENING
            cct <= 4600 -> LightingLabel.NEUTRAL_DAY
            else -> LightingLabel.DAYLIGHT
        }
...
/**
 * Ступень освещения — чистая логика без слов.
 *
 * ════════════════════════════════════════════════════════════════════════
 *  ЗАЧЕМ ОТДЕЛЬНОЕ ПЕРЕЧИСЛЕНИЕ, А НЕ СТРОКА
 * ════════════════════════════════════════════════════════════════════════
 *
 * Ступень нужна в двух местах: в лампе на главном экране и в карточке света
 * в настройках. Оба показывают её **словами**, но слова живут в ресурсах —
 * значит, логика обязана отдавать не слово, а решение.
 *
 * Границы подобраны по физиологии, а не по круглым числам:
 *
 *  — [CANDLE] до 2100 K — пламя, ниже порога 2200 K из вечерних
 *    рекомендаций. Мелатонин не подавляется вообще;
 *  — [WARM_LAMP] до 2500 K — тёплая лампа накаливания;
 *  — [SUNDOWN] до 3100 K — низкое солнце, золотой час;
 *  — [EVENING] до 3900 K — вечернее освещение, ещё тёплое, но уже не оранжевое;
 *  — [NEUTRAL_DAY] до 4600 K — нейтральный день;
 *  — [DAYLIGHT] выше — дневной свет, максимум меланопического вклада.
 */
enum class LightingLabel {
    CANDLE,
    WARM_LAMP,
    SUNDOWN,
    EVENING,
...
/**
 * Строковый ресурс для ступени света — мост между логикой и ресурсами.
 *
 * ════════════════════════════════════════════════════════════════════════
 *  ПОЧЕМУ ЭТА ФУНКЦИЯ ЖИВЁТ ЗДЕСЬ, А НЕ В РЕСУРСАХ
 * ════════════════════════════════════════════════════════════════════════
 *
 * `stringResource(id)` принимает `@StringRes Int`. Проверка разметки
 * гарантирует только одно: по этому адресу лежит строка. **Не** гарантию
 * того, что это правильная строка. Значит, связь «ступень → ресурс» должна
 * быть видна в одном месте и написана так, чтобы её нельзя было забыть:
 * `when` без `else` не скомпилируется, если появится новая ступень.
 *
 * ## Почему не наоборот (текст прямо в перечислении)
 *
 * Соблазн положить строку в константу есть, но это ломает главное: текст
 * обязан проходить через ресурсы, чтобы попадать в перевод. Enum с текстом —
 * тот же хардкод, только аккуратнее оформленный. Ровно из-за него при
 * восьми не-русских локалях приложения пользователь и видел русское «Закат».
 *
 * ## Про фолбэк на английский
 *
 * Требование: русский, английский и японский — с переводом, все прочие
 * локали — на английском. Это делается **без единого условия в коде**:
 * строки объявлены в `values/` (русский — язык по умолчанию проекта),
 * `values-en/` и `values-ja/`. Android сам берёт `values-en/` для любой
 * локали, перевода для которой нет. Код всегда один и тот же, решения
 * о языке не принимает — и потому ошибиться в нём не может.
 */
@get:StringRes
val LightingLabel.stringRes: Int
    get() = when (this) {
        LightingLabel.CANDLE -> R.string.light_candle
        LightingLabel.WARM_LAMP -> R.string.light_warm_lamp
        LightingLabel.SUNDOWN -> R.string.light_sundown
        LightingLabel.EVENING -> R.string.light_evening
        LightingLabel.NEUTRAL_DAY -> R.string.light_neutral_day
        LightingLabel.DAYLIGHT -> R.string.light_daylight
    }

/**
 * Фаза суток для расчётов и превью — без слов.
 *
 * Раньше каждая константа несла русскую подпись прямо в конструкторе. Она
 * нигде не показывалась пользователю (подписи фаз в интерфейсе берутся из
 * ресурсов), но создавала ровно ту ловушку, из-за которой в приложении уже
 * был непереводимый текст: кажется, что «текст здесь свой, локальный», и
 * следующий разработчик берёт его для экрана. Поэтому подписей нет вовсе —
 * нечего случайно показать.
 */
