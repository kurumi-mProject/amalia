...
/**
 * Цвета частиц мотива в живом слое.
 *
 * ## Почему мотив красится СОБСТВЕННЫМИ цветами, а не акцентами палитры
 *
 * Раньше [MotifLayer] брал `palette.motifAccent` — то есть цвета декораций,
 * привязанные к палитре текущего времени суток. Для [AmaliaMotif.AUTO] это
 * одно и то же (палитра и мотив решаются вместе), но для ручного выбора
 * это ломало всю идею: выбранные «Сакура» ночью рисовали лепестки
 * звёздными бледно-голубыми, «Клены» вечером — светлячковым лаймом.
 * Пользователь выбирал тему — и не узнавал её результат. Тема «не работала».
 *
 * Канонические пары дают каждому мотиву его собственную палитру — розовую
 * у сакуры, медную у клёна, серебряную у снега, — в любое время суток.
 * Плотность и альфа по-прежнему берутся из темы, поэтому частицы остаются
 * частью света, а не наклейкой.
 */
fun AmaliaMotif.particleColors(): Pair<Color, Color> = previewColors()
