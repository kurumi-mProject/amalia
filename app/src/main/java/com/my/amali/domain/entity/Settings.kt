...
data class WaveSettings(
     ...
     *
     * 2.4 вместо прежних 1.6: замер уровня идёт от RMS кадра,
     * нормированного на 8000 из 32767. На этом делителе обычная речь
     * вблизи микрофона даёт 0.3–0.6, но на расстоянии вытянутой руки —
     * 0.12–0.25, и при 1.6 полосы поднимались едва на четверть высоты:
     * волна выглядела сломанной, хотя звук был. При 2.4 обычная речь
     * качает полосы в среднюю треть, громкая — до потолка, и индикатор
     * читается живым при любом расстоянии от микрофона.
     */
    val sensitivity: Float = 2.4f,
