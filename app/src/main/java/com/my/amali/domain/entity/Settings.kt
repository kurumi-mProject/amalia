...
data class WaveSettings(
    val spikeCount: Int = 31,
    val spikeWidth: Float = 5f,
    val spikeGap: Float = 10f,
    val maxHeight: Float = 44f,
    val cornerRadius: Float = 1.5f,

    /**
     * Множитель реакции на громкость.
     *
     * 2.2 — середина между «шёпот виден» и «гром не искажён»: обычная
     * речь качает полосы в средней трети, крик достаёт до потолка.
     * Уточняется ползунком на экране волны.
     */
    val sensitivity: Float = 2.2f,

    /**
     * Сглаживание на максимуме (0.6) — отклик без задержки.
     *
     * Значение — доля пути к цели за кадр 60 Гц: при 0.6 полоса доходит
     * до нового уровня за ~2–3 кадра (35–50 мс), что глаз читает как
     * «волна звучит вместе с голосом». Меньшие значения — плавнее и
     * инерционнее (0.05 ≈ живая лава, 0.6 ≈ точный индикатор).
     */
