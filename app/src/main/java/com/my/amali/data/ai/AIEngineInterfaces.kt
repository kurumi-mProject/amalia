...
interface SpeechToTextEngine {
    ...
    /**
     * Готовит движок к работе заранее — вызывается при касании кнопки
     * микрофона, до самого старта записи.
     *
     * Нужен движкам, у которых есть что готовить: открыть соединение,
     * прогреть модель. Текущий распознаватель подключается самим запросом
     * и в прогреве не нуждается, поэтому оставлен пустой реализацией —
     * контракт сохраняется для будущих движков.
     */
...
sealed interface LLMEvent {
    ...
    /**
     * Текст ответа, восстановленный из невалидного JSON.
     *
     * Обычно ответ приходит как [ContentDelta] с сырым JSON, а движок
     * разбирает его сам. Но когда модель нарушает контракт (обрыв строки,
     * проза вместо JSON), сырой поток прочитать нельзя — и текст ответа
     * был бы потерян целиком, хотя он уже сгенерирован. Это событие несёт
     * уже готовую человеческую реплику.
     */
    data class ReplyRecovered(val text: String) : LLMEvent
}
