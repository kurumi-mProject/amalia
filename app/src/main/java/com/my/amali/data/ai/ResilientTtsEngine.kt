 ...
 * ═══════════════════════════════════════════════════════════
 *
 * Конвейер Амалии заканчивается фразой `ttsEngine.speak(...)`. Раньше
 * единственным движком был Fish Audio на бесплатной модели, и это давало
 * два реальных отказа (оба наблюдались на живом ключе):
 *
 ...
 *     разогревалась **~38 секунд**, хотя в прогретом состоянии отвечала
 *     за 2.4–3 с. Для голосового ассистента первая цифра — это «не
 *     работает»: пользователь успевает решить, что приложение сломалось.
 *
 * Сейчас основной движок — `drama-3-preview` (грант Fish Audio для
 * стартапов): первый байт приходит за 0.5–1.4 с, поэтому страховка
 * срабатывает уже не «каждый день», а только на настоящих сбоях сервиса.
 * Тем не менее она остаётся: ассистент не имеет права замолчать из-за
 * чужих проблем с балансом, сетью или очередью синтеза.
 *
