...
internal object VoiceAudio {
    ...
    /**
     * Уровень для отрисовки волны — тот же кадр, растянутый к верхней трети.
     *
     * [RMS_FULL_SCALE] калиброван «громкая речь вплотную к микрофону»,
     * поэтому обычный разговор на расстоянии даёт 0.15–0.5 — при таких
     * числах полосы поднимаются на четверть высоты, и волна читается
     * сломанной, хотя звук есть. Волна — индикатор, а не измеритель:
     * её шкала имеет право быть растянутой. Делитель 0.6 означает:
     * обычная речь качает полосы в средней трети, крик достаёт до потолка,
     * шёпот — заметно выше тишины.
     *
     * Важно: функция только для отображения. Детектор конца фразы
     * ([SpeechGate]) и запись в WAV обязаны видеть исходный [level] —
     * усиление не должно влиять на то, когда фраза считается сказанной.
     */
    fun displayLevel(frame: ShortArray): Float =
        (level(frame) / DISPLAY_LEVEL_CEILING).coerceIn(0f, 1f)

    /** Практический потолок громкости в шкале волны, 0..1. */
    const val DISPLAY_LEVEL_CEILING = 0.6f
