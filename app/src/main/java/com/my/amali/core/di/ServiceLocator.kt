...
object ServiceLocator {
     ...
     * и ту же строку.
     *
     * Лежит прямо в объекте, а не в `companion object`: [ServiceLocator] —
     * это `object`, у него нет и не может быть компаньона (компилятор
     * отвечает «Modifier 'companion' is not applicable inside 'standalone
     * object'»). Приватная константа в `object` видна только внутри него —
     * ровно то поведение, которое здесь и нужно.
     */
