...
```
                   ┌──────────────────────────────────────────────────────┐
                   │         cancelConversation() / кнопка Стоп           │
                   │                                                       │
                   ▼                                                       │
                [ IDLE ] ◀──────────────── аудио закончилось ──────────┐  │
                   │                                                    │  │
          нажатие на микрофон                                           │  │
          (isBusy=false)                                                │  │
                   │                                                    │  │
                   ▼                                                    │  │
             [ СЛУШАЮ ] ── тишина 600мс ──▶ [ ДУМАЮ ] ──▶ [ ГОВОРЮ ]
                   │                            │               │
          нажатие на микрофон            LLM ответил         playJob
          (isBusy=true)                  TTS стартует        завершился
                   │                                               │
                   └──────────── cancelConversation() ─────────────┘

  ОШИБКА ──▶ IDLE  (сбрасывается автоматически через 3 сек)
```
